<?php
	$filter = mysql_real_escape_string(utf8_decode($_GET["date"]));
	$appid = mysql_real_escape_string($_GET["appid"]);
	$fixed = mysql_real_escape_string($_GET["fixed"]);
	
	$result = mysql_query("SELECT * FROM exceptions WHERE `Date` LIKE '$filter%' AND `App`='$appid' AND `Fixed`='$fixed'") or die(mysql_error());
